...
export class FiltersComponent {
  constructor(private CommunitService: CommunityServiceService, private fb: FormBuilder) {
  ...
  ngOnInit() {    
    ...
    this.CommunitService.getdistrictmaster()
      .subscribe(item => {
    ...
    this.CommunitService.gethudmasterlooker()
      .subscribe(item => {
    ...
    this.CommunitService.getblockmasterlooker()
      .subscribe(item => {
    ...
    this.CommunitService.getrolemasterlooker()
      .subscribe(item => {
    ...
    this.CommunitService.getdirectoratemasterlooker()
      .subscribe(item => {
  ...
  PayloadDetails() {
    ...
    if (this.form.value.districtmyItems != "" && this.form.value.districtmyItems != null) {
      for (var i = 0; this.form.value.districtmyItems.length > i; i++) {
        DistrictParam = DistrictParam + this.form.value.districtmyItems[i].district_id + ",";
      }
      payload['district_id'] = DistrictParam.substring(0, DistrictParam.length - 1);
    }

    if (this.form.value.hudmyItems != "" && this.form.value.hudmyItems != null) {
    ...
    if (this.form.value.blockmyItems != "" && this.form.value.blockmyItems != null) {
      for (var i = 0; this.form.value.blockmyItems.length > i; i++) {
        blockParam = blockParam + this.form.value.blockmyItems[i].block_id + ",";
      }
      payload['block_id'] = blockParam.substring(0, blockParam.length - 1);
    }

    if (this.form.value.facilitymyItems != "" && this.form.value.facilitymyItems != null) {
      for (var i = 0; this.form.value.facilitymyItems.length > i; i++) {
        FacilityParam = FacilityParam + this.form.value.facilitymyItems[i].facility_id + ",";
      }
      payload['facility_id'] = FacilityParam.substring(0, FacilityParam.length - 1);
    }

    if (this.form.value.institutiondistrictmyItems != "" && this.form.value.institutiondistrictmyItems != null) {
      for (var i = 0; this.form.value.institutiondistrictmyItems.length > i; i++) {
        InstitutiondistrictParam = InstitutiondistrictParam + this.form.value.institutiondistrictmyItems[i].district_id + ",";
      }

      payload['indistrict_id'] = InstitutiondistrictParam.substring(0, InstitutiondistrictParam.length - 1);
    }

    if (this.form.value.institutionhudmyItems != "" && this.form.value.institutionhudmyItems != null) {
      for (var i = 0; this.form.value.institutionhudmyItems.length > i; i++) {
        InstitutionhudParam = InstitutionhudParam + this.form.value.institutionhudmyItems[i].hud_id + ",";
      }

      payload['inhud_id'] = InstitutionhudParam.substring(0, InstitutionhudParam.length - 1);
      
    }

    if (this.form.value.institutionblockmyItems != "" && this.form.value.institutionblockmyItems != null) {
      for (var i = 0; this.form.value.institutionblockmyItems.length > i; i++) {
        InstitutionblockParam = InstitutionblockParam + this.form.value.institutionblockmyItems[i].block_id + ",";
      }

      payload['inblock_id'] = InstitutionblockParam.substring(0, InstitutionblockParam.length - 1);
    }

    if (this.form.value.villagemyItems != "" && this.form.value.villagemyItems != null) {
      for (var i = 0; this.form.value.villagemyItems.length > i; i++) {
        villageParam = villageParam + this.form.value.villagemyItems[i].village_id + ",";
      }
      payload['village_id'] = villageParam.substring(0, villageParam.length - 1);
    }


    if (this.form.value.directoratemyItems != "" && this.form.value.directoratemyItems != null) {
      for (var i = 0; this.form.value.directoratemyItems.length > i; i++) {
        DirectorateParam = DirectorateParam + this.form.value.directoratemyItems[i].directorate_id + ",";
      }
      payload['directorate_id'] = DirectorateParam.substring(0, DirectorateParam.length - 1);
    }


    if (this.form.value.rolemyItems != "" && this.form.value.rolemyItems != null) {
    ...
    if (this.form.value.institutionfacilitymyItems != "" && this.form.value.institutionfacilitymyItems != null) {
      for (var i = 0; this.form.value.institutionfacilitymyItems.length > i; i++) {
        InstitutionfacilityParam = InstitutionfacilityParam + this.form.value.institutionfacilitymyItems[i].facility_id + ",";
      }
      payload['infacility_id'] = InstitutionfacilityParam.substring(0, InstitutionfacilityParam.length - 1);
    }
  ...
  onSearch(evt: any) {
    ...
    this.CommunitService.getvillagemasterlooker(querystring)
      .subscribe(item => {
  ...
  onSearchFacility(evt: any) {
    ...
    this.facilitylist = [];
    var querystring = evt.target.value;
    this.CommunitService.getfacilitymasterlooker(querystring)
      .subscribe(item => {
